classdef MacquarieIMUBiomechanicsApp < handle
    %
    % Usage:
    %     app.MacquarieIMUBiomechanicsApp
    ...
    properties (Constant)
        AppPathBase = {'/MacquarieIMUBiomechanics'};
        AppClass = 'MacquarieIMUBiomechanicsApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function obj = MacquarieIMUBiomechanicsApp()
        ...
        function startApp(obj)
            ...
            mlock;
            MacquarieIMUBiomechanicsApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'mqAppTabs')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong mqAppTabs
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'mqAppTabs').getString, ...
            ...
            if nargout(@mqAppTabs) == 0
                eval('mqAppTabs');
            else
                obj.AppHandle = eval('mqAppTabs');
            end
